...
import {get_length} from '../functions/getlenghtId'
...
function Center() {
    ...
    const getInvoiceData = async () => {
        let dataList = [];
         let len = await get_length();
         console.log("🚀 ~ getInvoiceData ~ len  0000000000000000000:", len)
        for (let i = 0; i < len; i++) {
            console.log("i", i);
            let data = await get_invoice(i);
            dataList.push(data);
            // console.log("🚀 ~ getInvoiceData ~ dataList:", dataList);
        }
        setInvoicesList(dataList); // Update state with new data list
    };

    const click = () => {
        // console.log("Current invoicesList:", invoicesList);
        invoicesList.map((a) => {
            console.log("🚀 ~ invoicesList.map ~ a:", a);
        });
    };
