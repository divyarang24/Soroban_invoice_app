...
function encode(data) {
    ...
    if (lengthWithoutPadding > 0) {
        ...
        for (let i = 0; i < lengthWithoutPadding; i += 5) {
            ...
            let buffer = '';
            for (let s = 0; s < 8; s += 1) {
                const segment = Number(bits & BigInt( 0b11111))
...
function decode(data) {
    if (data.length % 8 !== 0) {
        throw 'Error: decode expects a valid Base32 encoded string, this one has invalid length.';
    }
    ...
    for (let i = 0; i < data.length; i += 8) {
        ...
        for (let b = 0; b < unpaddedBytes; b += 1) {
            buffer.push(
                Number((bits >> BigInt(8 * b + gap)) &BigInt( 0b11111111))
            );
        }
